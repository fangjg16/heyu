...
登录/注册走 [Clerk](https://clerk.com/)（保持现有合域登录页）。请在 Clerk Dashboard 打开 **Email + Password**，并把 `fangjg16.github.io`、`localhost:5173` 加到允许来源。ECS `.env` 必须填 `CLERK_SECRET_KEY`（`sk_...`），`CLERK_PUBLISHABLE_KEY` / `CLERK_JWT_ISSUER` 见 `.env.example`。然后重建 `jfo-api` 并执行 `deploy/ecs/scripts/migrate.sh`。
...
| jfo-api 日志 `internal error; reference = …` 或 `DNS lookup failed; params.host = hermes` | workerd 不能直连 Compose 里的 Hermes。`git pull` 后 `up -d --build jfo-api`；启动日志应有 `Hermes 出站走 Node 反代: Worker http://127.0.0.1:8791/__jfo/internal/hermes -> http://hermes:8642`。`/api/health` 里 `hermesViaNodeProxy` 应为 true，`hermesAuthOk` / `hermesRunsOk` 应为 true |
