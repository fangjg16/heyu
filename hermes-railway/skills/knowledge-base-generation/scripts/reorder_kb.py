#!/usr/bin/env python3
"""Reorder KB display order without editing content panels.

Usage:
  python reorder_kb.py input.html output.html --order snapshot,assets,returns,risks,... --reason "user request"
"""
...
from kb_config import CANONICAL, load, replace_config
...
def reorder_nav(html: str, order: list[str]) -> str:
    allowed = {"overview", "source-index", "glossary", *CANONICAL}
    items = {
        m.group("target"): m.group(0)
        for m in NAV_RE.finditer(html)
        if m.group("target") in allowed
    }
    ...
    for key in order:
        if key in items:
            num = NUMERALS[order.index(key)] if order.index(key) < len(NUMERALS) else str(order.index(key) + 1)
            rendered.append(
                re.sub(
                    r'(<span class="kb-nav-num">).*?(</span>)',
                    rf"\g<1>{num}\2",
                    items[key],
                    count=1,
                    flags=re.S,
...
def renumber_sections(html: str, order: list[str]) -> str:
    number_for = {key: NUMERALS[i] for i, key in enumerate(order) if i < len(NUMERALS)}
    def repl(m: re.Match[str]) -> str:
        key = m.group("id")
        if key not in number_for:
            return m.group(0)
        return m.group(1) + number_for[key] + m.group(4)
...
def main() -> None:
    ...
    p.add_argument("--order", required=True, help="comma-separated canonical slot keys")
    p.add_argument("--reason", default="用户调整展示顺序")
    args = p.parse_args()
    ...
    cfg.history.append(f"{date.today().isoformat()} | reset | {args.reason}")
