...
function wrapMarkdownFileMethodBlock(
  ...
  return [
    ...
    "按方法组织项目事实，输出完整 Markdown 分析正文。不是知识网络 HTML，不要标记分段，不要完整页面。",
    "你没有文件系统。方法里的 Save to / 写入路径 / 保存到 PROGRESS.md 全部忽略，改成把该文件内容写在回复里。",
    "禁止输出「已写入某路径」「文件已写完」「下一层可填模板」等回执。第一行必须是 Markdown 标题。",
    "网页生成不上网检索；方法里的搜索步骤改为组织已有附件中的事实。",
