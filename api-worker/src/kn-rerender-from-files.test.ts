...
describe("rerender knowledge chapters from files", () => {
  ...
  it("covers mature research chapters that render from files", () => {
    ...
    expect(ids).toContain("risk-return");
    expect(ids).toContain("diligence-gaps");
    expect(ids).not.toContain("industry-overview");
    expect(ids).not.toContain("company-background");
    expect(ids).not.toContain("assumption-validation");
