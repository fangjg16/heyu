...
const SECTION_FORMAT_HINT: Record<string, string> = {
  "project-summary":
    "===CHAPTER=== 1.1 结论判断条（筛选用 Exciting/Pass 等；尽调用 Proceed/Reject 等）+ 工件状态 + 项目摘要 + 交易概况。保留 class。证据列只写 [A-1]。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "industry-overview":
  ...
  "industry-competition":
    "===CHAPTER=== 本章结论 + 2.1–2.5 行业概况/需求/产业链/竞争结构/趋势。筛选阶段用公开信息，尽调阶段用行业底稿。不要对战卡。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "business-overview":
  ...
  "business-technology":
    "===CHAPTER=== 本章结论 + 业务/产品/技术/商业模式/核心竞争力。禁止 IRR。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "company-team":
    "===CHAPTER=== 本章结论 + 公司基本信息 + 背景调查要点 + 主体/个人表 + 红黄旗。禁止 SVG。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "company-background":
    "===CHAPTER=== 判断条 + 背景调查要点 + 主体/个人表 + 红黄旗。禁止 SVG。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "financial-diligence":
    "===CHAPTER=== 本章结论 + 工件状态 + 收入/毛利率/现金数字条 + 5.1 数据口径 + 5.2 测算 + 5.3 假设 + 账实质量。筛选阶段禁止 IRR。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  ...
  "risk-return":
    "===CHAPTER=== 本章结论 + 6.1 估值回报（筛选可 indicative）+ 6.2 主张核验 + 6.3 投资风险热力图与登记。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "diligence-gaps":
    "===CHAPTER=== 判断条 + 7.1 问被投方、7.2 内部核验与判断，各组 kn-fold。禁止改成三列表。保留 class。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  ...
  benchmarks:
    "===CHAPTER=== 只填模板里出现的块（服务端已按项目形态去掉另一套）。早期：功能矩阵 + 定价 + 3–5 张对战卡，无成交则不要出价大数字。投资：出价区间 + 可比交易（含经营差异列）+ 溢价/折价。禁止两套并排。附件点名的对标主体必须进入矩阵或对战卡；禁止只用通用品类/海外工具示例顶替。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  business:
    "===CHAPTER=== 只填模板里出现的那一张画布。早期：Lean Canvas 宫格；有交付才填短 Journey。投资：Journey + BMC。再填客户表、单位经济、待验证假设。禁止 IRR/MOIC。保留内联 grid style。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  returns:
    "===CHAPTER=== 按模板：文首标明假设模型或已校准 + 回报摘要 + 估值大数字 + 三情景卡 + 看板/现金流/敏感性。情景卡必须保留，禁止 SVG。缺数字写待补。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  capabilities:
    "===CHAPTER=== 按模板：关系摘要 + 对手方/顾问/关键个人三卡 + 关系表。不要公开检索总表。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  ownership:
  ...
  framework:
    "===CHAPTER=== 按模板：决策建议 callout + 双列条件卡（投资：推进/暂缓；早期：出资/不追投）+ 论点 + 法律路径 + 杠杆 + 路线图 + 下一步。不要 Top5 风险表和三情景 IRR 摘要。随后 ===SOURCES_ADD=== / ===GLOSSARY_ADD===。",
  "project-overview":
...
export async function handleGenerateProjectKnowledgeChapter(
  ...
  const userPrompt = [
    ...
    "摘录/说明列可写资料「说明」（人写的用途）；仍用 [A-n] 对文件，不要另造引用种类。",
    SOURCES_TABLE_SKELETON,
    "",
