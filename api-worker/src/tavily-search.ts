...
export function tavilyCapabilitySystemLines(configured: boolean): string[] {
  if (!configured) {
    return [
      "联网公开信息：当前未配置（TAVILY_API_KEY）。仍可基于【项目知识】与合理推论作答；若用户需要网页核实，说明联网能力待开通。",
    ];
  }
  return [
    "联网公开信息：已接入。公开检索由对话引擎在需要时自行发起，不要让用户背「查外部资料」口令。",
    "若用户只问「能不能查外部」，回答：可以，直接说要查的主题或链接即可。",
    "若本轮含【外部检索】：须先用 [WEB:n]+URL 概括网页要点，再与【项目知识】对照。勿写「我无法上网」。",
