...
export function buildHermesAgentInstructions(
  ...
  const lines = [
    ...
    ...(isOpenChat
      ? [
          "你处在本项目里回答。下方注入的【项目知识】是上传解析后已经掌握的内容，不是本轮临时检索流程。",
          "- 寒暄、短问、基于项目事实的判断：直接用这些知识作答。",
          "- 不要为了「显得读过资料」去 curl 清单或全文。",
          "- 知识盖不住、或用户明确要尽调/清单/IC 等专项交付：再 search / textUrl / 对应 skill。",
          "- 禁止为闲聊生成整页知识网络 HTML。",
        ]
      : [
          "1. 先用已注入的项目知识作答与分析（上传时已解析）。不够再用 jfo-r2-materials 补读缺口文件 / 当前 KB。",
