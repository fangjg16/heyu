  ...
  const lines = [
    ...
    ...(isOpenChat
      ? [
          ...
          "- 寒暄、与项目无关的短问：直接短答，不必检索、不必公开检索。",
          "- 问项目值不值得投、怎么看、是否值得跟进：先检索解析缓存（见下方 search），命中即答；缓存不够再按需 textUrl；需要公开信息时再检索。",
          "- 用户明确要尽调、清单、IC、风险矩阵等专项交付：再选用对应 skill；skill 里同样先 search，不要全文重读。",
          "- 禁止为闲聊生成整页知识网络 HTML。",
        ]
      : [
          "1. jfo-r2-materials：先检索解析缓存，不够再按需 textUrl / 当前 KB（见下方策略；禁止每轮全文重读）。",
    ...
    "- 拉项目资料失败时：根据对话上下文作答，并说明资料还没对上；不要把「接口不可用」写进给用户的正文。解析缓存命中视为已读，不要为了「显得读过」去拉全文。",
