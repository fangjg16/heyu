...
function TableCellContent({ children, userVariant }: { children: React.ReactNode; userVariant: boolean }) {
  ...
  if (statusMatch) {
    ...
    return (
      <span
        className={cn(
          "inline-flex max-w-full flex-wrap items-start gap-1 rounded-lg border px-1.5 py-0.5 text-[13px] font-medium leading-snug",
  ...
  return (
    ...
    >
      ...
      <span
        ...
        role="tooltip"
        className={cn(
          "pointer-events-none absolute z-30 w-max max-w-[min(22rem,calc(100vw-16px))] rounded-lg border px-2 py-1.5 text-[13px] leading-snug shadow-lg",
...
export function ChatMarkdown({ text, variant }: ChatMarkdownProps) {
  ...
  const components: Components = {
    ...
    h1: ({ children }) => (
      <h1
        className={cn(
          "mb-2 mt-1 text-[15px] font-semibold",
    ...
    h2: ({ children }) => (
      <h2
        className={cn(
          "mb-3 mt-5 border-b pb-2 text-[15px] font-semibold tracking-tight first:mt-0",
