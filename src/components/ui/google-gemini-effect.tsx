...
function strokeOffset(draw: number): number {
  const visible = Math.min(1, Math.max(0, draw));
  return 1 - visible;
}
...
export function GoogleGeminiEffect({
  ...
  /** 五条线的「绘制量」0~1.2+，与原先 framer pathLength 一致 */
  pathDraw: [number, number, number, number, number];
  /** 字符串为原 Aceternity 灰阶大标题；`ReactNode` 用于落地页 Hero 同款双行标题 */
  title?: ReactNode;
  description?: ReactNode;
  className?: string;
  cta?: ReactNode;
  /** `hero`：嵌入首屏中间，仅保留曲线与轻提示 */
  variant?: "full" | "hero";
}) {
  const blurFilterId = `gf-blur-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;

  const [d0, d1, d2, d3, d4] = pathDraw;
  ...
  const svgInner = (
    <>
      <path
        ...
        pathLength={1}
        strokeDasharray="1"
        strokeDashoffset={strokeOffset(d0)}
      />
      <path
        ...
        pathLength={1}
        strokeDasharray="1"
        strokeDashoffset={strokeOffset(d1)}
      />
      <path
        ...
        pathLength={1}
        strokeDasharray="1"
        strokeDashoffset={strokeOffset(d2)}
      />
      <path
        ...
        pathLength={1}
        strokeDasharray="1"
        strokeDashoffset={strokeOffset(d3)}
      />
      <path
        ...
        pathLength={1}
        strokeDasharray="1"
        strokeDashoffset={strokeOffset(d4)}
      />
  ...
  if (variant === "hero") {
    /** 裁掉原图上方空白，使 `meet` 时以宽度为基准缩放，曲线与按钮同层、不会「顶天立地」 */
    const heroViewBox = "0 330 1440 400";

    return (
      <div className={cn("relative w-full", className)}>
        <p className="mb-2 text-center text-[0.65rem] font-mono uppercase tracking-[0.14em] text-sky-400/90">
          能力曲线 · 随滚动点亮
        </p>
        <div className="relative w-full">
          <div className="relative aspect-[1440/400] w-full overflow-hidden rounded-sm">
            <svg
              viewBox={heroViewBox}
  ...
  const defaultTitle = "Build with Aceternity UI";
  const defaultDesc =
    ...
    >
      <div className="mx-auto w-full max-w-4xl px-4 pb-6 md:px-6 md:pb-9 lg:pb-10">
        {typeof title === "string" || title == null ? (
          ...
          <h1 className="font-display text-[clamp(2.2rem,5.5vw,3.5rem)] font-extralight leading-[1.08] tracking-[-0.04em] text-foreground pb-4 text-center">
            {title}
          </h1>
        )}
        {typeof description === "string" || description == null ? (
          <p className="mx-auto mt-4 max-w-lg text-center text-xs font-normal text-neutral-400 md:text-xl">
            {(description as string | undefined) ?? defaultDesc}
          </p>
        ) : (
          <div className="mx-auto mt-4 max-w-2xl space-y-4 text-center text-[0.85rem] font-light leading-relaxed text-muted-foreground md:text-[0.92rem] md:leading-relaxed">
            {description}
          </div>
        )}
      </div>
