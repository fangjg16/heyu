...
function MessageBubbleToolbar({
  ...
  const actionBtnClass =
    "rounded-lg p-1.5 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100";
...
export default function ConversationCenter() {
  ...
  /** Last successful hydrate size; auto-PUT is skipped if local list is clearly smaller. */
  const lastHydratedConversationCountRef = useRef(0);
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      const applyHydrated = (
        incomingConvs: SessionConversation[],
        ...
        setConversations((prev) => {
          ...
          if (writeCache) {
            ...
            if (
              ...
              !isTruncatedConversationList(
                merged.length,
                prevCache.conversations.length,
              )
            ) {
      ...
      if (remote) {
        ...
        applyHydrated(next, messagesByConversation, true);
      ...
      if (cached && (cached.conversations.length > 0 || cachedMsgCount > 0)) {
        ...
        applyHydrated(next, cachedMsgs, false);
        setChatSyncError(
  ...
  useEffect(() => {
    ...
    if (ask.wantNew) {
      ...
      skipNextAutoPersistRef.current = true;
      setConversations((prev) => {
        const cached = SESSION_CONVERSATION_CACHE[userId]?.conversations ?? [];
        const base = prev.length >= cached.length ? prev : cached;
        return prependConversation(base, newConv);
      });
      setLiveMessagesByConversation((prev) => ({
        ...prev,
        [newId]: prev[newId] ?? [],
  ...
  useEffect(() => {
    if (!userId || !chatSyncReady) return;
    const prevCache = SESSION_CONVERSATION_CACHE[userId];
    if (
      !prevCache ||
      !isTruncatedConversationList(
  ...
  const handleSend = async () => {
    ...
    if (isLiveAiMode && project && !conversations.some((c) => c.id === effectiveConversationId)) {
      ...
      setConversations((prev) => prependConversation(prev, blankConv));
    }
          ...
          <div className="flex w-full items-center justify-between gap-2">
            <button
              ...
              onClick={() => {
                ...
                skipNextAutoPersistRef.current = true;
                setConversations((prev) => prependConversation(prev, newConv));
                setLiveMessagesByConversation((prev) => ({
                  ...prev,
                  [newId]: prev[newId] ?? [],
                }));
                            ...
                            <>
                              ...
                              >
                                <button
                                  ...
                                  className="rounded-lg p-1 text-muted-foreground hover:bg-[hsl(var(--wine)/0.08)] hover:text-[hsl(var(--wine))]"
                                >
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[rgba(255,252,248,0.55)]">
        <header className="sticky top-0 z-10 flex min-h-[3.25rem] flex-wrap items-center justify-between gap-2 border-b border-border/50 bg-white/70 px-4 py-2 backdrop-blur-md md:px-6">
          ...
          >
            ...
            {showHistoryMenu ? (
              <div className="absolute right-0 top-9 z-20 w-[17rem] rounded-xl border border-border/70 bg-white/95 p-1.5 shadow-[0_8px_24px_-12px_rgba(15,23,42,0.28)] backdrop-blur-md">
        ...
        >
          ...
          <div
            className={cn(
              "relative flex w-full min-w-0 items-end gap-0.5 rounded-xl border border-black/[0.08] bg-white py-1 pl-3 pr-1 focus-within:border-[hsl(var(--wine-deep)/0.28)] focus-within:ring-1 focus-within:ring-[hsl(var(--wine-deep)/0.16)]",
          ...
          >
            ...
            {composerDragActive ? (
              <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-white/92 text-[13px] font-medium text-[hsl(var(--wine-deep))]">
                松开以添加
              </div>
            ) : null}
          </div>
